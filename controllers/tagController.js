  ...
  async.parallel(
    {
      tag: function (callback) {
        Tag.findByIdAndUpdate(req.params.id, { $inc: { popularity: 1 } }).exec(
          callback
        );
      },
      ...
      tag_users: function (callback) {
        User.find({ tag: req.params.id }).exec(callback);
      },
    },
    function (err, results) {
      ...
      res.render("tag_detail", {
        ...
        current_user: req.user,
        tag: results.tag,
        tag_jobs: results.tag_jobs,
        tag_users: results.tag_users,
        followed: followed,
      });
    }
  );
  ...
  async.parallel(
    {
      current_user: function (callback) {
        User.findByIdAndUpdate(req.user.id, {
          $push: { followed_tag: req.params.id },
        })
...
// Redirect to Tag detail page when unfollow
exports.tag_unfollow_get = function (req, res, next) {
  async.parallel(
    {
      current_user: function (callback) {
        User.findByIdAndUpdate(req.user.id, {
          $pull: { followed_tag: req.params.id },
        })
          .populate("followed_tag")
          .exec(callback);
      },
